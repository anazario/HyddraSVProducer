#ifndef FastSimMuonEnhancedTracksProducer_cc
#define FastSimMuonEnhancedTracksProducer_cc
// -*- C++ -*-
//
// Package:    KUCMSNtupleizer/HyddraSVProducer
// Class:      FastSimMuonEnhancedTracksProducer
//
// Description: FastSim version of MuonEnhancedTracksProducer.
//              Same logic but without displacedTracks and displacedMuons inputs,
//              which are not available in FastSim AOD. Uses globalMuons and
//              standAloneMuons as proxies for the displaced muon collections.
//
...
class FastSimMuonEnhancedTracksProducer : public edm::stream::EDProducer<> {

public:
  explicit FastSimMuonEnhancedTracksProducer(const edm::ParameterSet&);
  virtual ~FastSimMuonEnhancedTracksProducer() = default;
...
FastSimMuonEnhancedTracksProducer::FastSimMuonEnhancedTracksProducer(const edm::ParameterSet& iConfig) :
...
void FastSimMuonEnhancedTracksProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace std;
  ...
  // FastSim: only globalMuons (dgMuonTracks) and standAloneMuons (dsaMuonTracks)
  // No displacedTracks available in FastSim
...
void FastSimMuonEnhancedTracksProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("generalTracks", edm::InputTag("generalTracks"));
  desc.add<edm::InputTag>("dsaMuonTracks", edm::InputTag("standAloneMuons"));
  desc.add<edm::InputTag>("dgMuonTracks", edm::InputTag("globalMuons"));
  desc.add<edm::InputTag>("pvCollection", edm::InputTag("offlinePrimaryVertices"));

  descriptions.addDefault(desc);
}

std::vector<reco::Track> FastSimMuonEnhancedTracksProducer::toTrackVec(const std::set<reco::TrackRef> &trackSet) const {
...
std::set<reco::TrackRef> FastSimMuonEnhancedTracksProducer::toTrackSet(const edm::Handle<reco::TrackCollection> &trackHandle) const {
...
reco::TrackRef FastSimMuonEnhancedTracksProducer::findRef(const reco::Track &track, const std::set<reco::TrackRef> &trackSet) const {
...
void FastSimMuonEnhancedTracksProducer::subMuonTracks(std::set<reco::TrackRef> &tracks, const std::set<reco::TrackRef> &muonTracks) const {
...
DEFINE_FWK_MODULE(FastSimMuonEnhancedTracksProducer);
